...
bks,kind FROM auth WHERE id={$ath}";
	...
	if($row = mysqli_fetch_assoc($query)) {
		...
		$title = $row["profname"] . " &lsaquo; " . $_TITLE;
...
,bks_completion,kind FROM auth WHERE id=$ath";    
	...
	if($info = mysqli_fetch_assoc($query)) {
		...
		if($query = mysqli_query($conn, $q)) {
			...
			$title = "{$bknow[$bk-1]} &lsaquo; {$info["profname"]} &lsaquo; $_TITLE";
...
{$_ids[0]} AND {$_ids[2]}";
			...
			if(mysqli_num_rows($query)>0 and
				...
				$title = "{$row[1]["name"]} &lsaquo; {$bknow[$bk-1]} &lsaquo; " .
					 "{$info["profname"]} &lsaquo; $_TITLE";
