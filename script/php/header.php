...
?>
	...
	<html dir='<?php echo $site_dir; ?>' lang='<?php echo $site_lang_cc; ?>'>
		<head>
			...
			<link rel='stylesheet'
			      href='<?php echo _R; ?>style/css/main-comp.css?v169'/>
			<title>
				<?php echo $title; ?>
			</title>
			<style>
...
body{font-family:'{$fname_}'}";
			 ...
			 if($_auto_theme) {
				 $_darks = $all_colors["dark"];
				 echo "@media(prefers-color-scheme:dark){body,.dd-frame,.search-main .dropdown-content{background:{$_darks[0]}}.button:hover,.fontsize button:hover,.toolbar a:hover,.icon-round,.color-white{color:{$_darks[0]}}body,input,textarea,button,a,.color-black,#hon sup:hover{color:{$_darks[1]}}.icon-round{background:{$_darks[1]}}.button:hover,.fontsize button:hover,.toolbar a:hover,.btn-selected,.back-blue,.loader,.loader-round{background:{$_darks[2]}}#hon sup,.poet:hover,.color-blue,.selected,.link-color,.link:hover,a:hover,button:hover,.search-main .cb:hover,.bks p,.poetdesc p,#search-res #bhon,.bhoh-newdaq,#main-contributing h1,#main-contributing h2,#main-contributing h3,.h::after{color:{$_darks[2]}}.border-blue,input:focus,textarea:focus,.link-underline,#main-contributing a{border-color:{$_darks[2]}}.color-red{color:{$_darks[3]}}.back-red{background:{$_darks[3]}}::-webkit-scrollbar-track{background:{$_darks[0]};}::-webkit-scrollbar-thumb{background:{$_darks[2]}}::selection{color:{$_darks[0]};background:{$_darks[2]}}::-moz-selection{color:{$_darks[0]};background:{$_darks[2]}}::placeholder{color:{$_darks[1]}}::-webkit-input-placeholder{color:{$_darks[1]}}::-ms-input-placeholder{color:{$_darks[1]}}}";
			 }
			 ?>
			 <?php
			 if(@$_back_img)
